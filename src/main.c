...
int main(int argc, char ** argv){
	...
	if(argc > 1){
		if(strcmp(argv[1] , "test") == 0){
			FILE * file = fopen("/tmp/pos_printer", "wb");
			fn_test_run(file);
			fclose(file);
		}
	}
	else
	{
		...
		char datestring[20];
		fn_emulator_set_date_string(datestring);

		fn_emulator_logger_log_message("\n\n\nEmulator started at: ", log_file);
		strcat(datestring, "\n");
		fn_emulator_logger_log_message(datestring, log_file);
